  ...
  <div class="app-shell">
    ...
    <nav class="navbar">
      ...
      <div class="nav-links" :class="{ 'mobile-open': isMenuOpen }">
        ...
        <!-- Theme Toggle -->
        <button class="theme-toggle" @click="toggleTheme" :title="isLightMode ? 'Modo Oscuro' : 'Modo Claro'">
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (isJson) {
                ...
                throw new Error("Local Dev env - API unavailable");
            }
        } else {
...
html.light-theme {
  ...
  --color-accent: #007BFF; /* Tech Blue para modo claro */
  --color-secondary: #CCCCCC;
  --color-text-main: #121212;
  --color-text-dim: #555555;
  --color-card-bg: rgba(255, 255, 255, 0.9);
  --color-nav-bg: rgba(245, 245, 245, 0.95); /* Header claro */
}

html {
  margin: 0;
  padding: 0;
  background-color: var(--color-bg); /* Forces the theme logic to apply deeply */
...
body {
  margin: 0;
  padding: 0;
  overflow-x: hidden; /* CORTA cualquier elemento como la marca de agua para no hacer barras dobles horizontales */
...
.navbar {
  ...
  width: 100%;
  top: 0;
  background: var(--color-nav-bg, rgba(18, 18, 18, 0.95)); /* Dynamic bg */
...
@media (max-width: 768px) {
    ...
    /* Drawer Menu (Slide from side on mobile) */
    .nav-links {
        position: fixed; /* Fixed prevents parent box dilation causing mobile auto-zoom */
        top: 60px; right: -250px; /* Hidden by default */
        width: 200px;
        height: auto; /* NO MORE 100vh! */
        padding: 20px;
        ...
        border-bottom: 2px solid var(--color-accent); 
        flex-direction: column;
        justify-content: flex-start; /* Do not spread them out! */
        align-items: center;
        transition: right 0.4s cubic-bezier(0.19, 1, 0.22, 1);
        z-index: 300;
        gap: 0px; /* Links have padding, so gap is 0 */
    }
    ...
    .nav-links a {
        ...
        padding: 15px 0; /* Boxy but compact */
    ...
    ::-webkit-scrollbar {
        display: none; /* Hide custom scrollbars on mobile to let native handle */
    }
